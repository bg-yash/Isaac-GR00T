"""
This file was originally provided by Agile. But feel free to modify it.
It creates python bindings to the C interface of the Diana API.
"""
...
errorCodeMessage = {
    ...
    -2305: "ERROR_CODE_ PLANNER_ERROR",
    -2306: "ERROR_CODE_ HOME_POSITION_ERROR",
...
class RobotState(Enum):
    ...
    IDLE = 2
    FREE_DRIVING = 3
    ZERO_SPACE_FREE_DRIVING = 4
    HOLD_BRAKE = 5
    ERROR = 6
    HANDLING_ERROR = 7  # outside joint limits?
    RESCUE_MODE = 8
...
class DIANA_JACOBI_MATRIX(Structure):
    _fields_ = [("values", c_double * 42)]


class DIANA_DIGTAL_VALUE_ALL(Structure):
    _fields_ = [("values", c_int * 8)]
...
class DIANA_ANALOG_MODE(Structure):
    _fields_ = [("values", c_int * 2)]


class DIANA_DH_R(Structure):
    _fields_ = [("values", c_double * 28)]
...
def moveL(
    ...
    ret = moveLToPose(
        pose, v, a, zv_shaper_order, zv_shaper_frequency, zv_shaper_damping_ratio
    )
    return message(ret)
...
def initDHCali(tcpMeas, jntPosMeas, nrSets, ipAddress=""):
    ...
    ret = api_mod.initDHCali(
        tcpMeasData, jntPosMeasData, nrSets, bytes(ipAddress.encode("utf-8"))
    )
    return message(ret)
...
def servoJ(joints_pos, t=0.01, ah_t=0.01, gain=300, ipAddress=""):
    ...
    ret = api_mod.servoJ(
        dianaJointsPosition, t, ah_t, gain, bytes(ipAddress.encode("utf-8"))
    )
    return message(ret)


def servoL(tcp_pose, t=0.01, ah_t=0.01, gain=300, scale=1.0, ipAddress=""):
    dianaTcpPose = DIANA_TCP_POSE()
    dianaTcpPose.values = (c_double * 6)()
    for index in range(0, len(dianaTcpPose.values)):
...
def servoL_ex(
    tcp_pose, t=0.01, ah_t=0.01, gain=300, scale=1000, realiable=False, ipAddress=""
):
    dianaTcpPose = DIANA_TCP_POSE()
    dianaTcpPose.values = (c_double * 6)()
    for index in range(0, len(dianaTcpPose.values)):
...
def speedJ_ex(speed, acc, t=0.0, realiable=False, ipAddress=""):
    ...
    ret = api_mod.speedJ_ex(
        dianaJointsSpeed, acc, t, realiable, bytes(ipAddress.encode("utf-8"))
    )
    return message(ret)
...
def dumpToUDisk(timeout_second, ipAddress=""):
    ...
    ret = api_mod.dumpToUDisk(timeout_second, bytes(ipAddress.encode("utf-8")))
    return message(ret)
...
def getWayPoint(
    waypointName, dblTcppos, dblJoints, strToolName, strWorkpieceName, ipAddress=""
):
...
def setWayPoint(
    ...
    ret = api_mod.setWayPoint(
        ...
        dianaTcpPose,
        dianaJointPos,
        bytes(strToolName.encode("utf-8")),
        bytes(strWorkpieceName.encode("utf-8")),
        bytes(ipAddress.encode("utf-8")),
    )
    return message(ret)
...
def getInverseClosedJoints(id, index, joints, ipAddress=""):
    ...
    size = api_mod.getInverseClosedJoints(
        id, index, dianaJoints, bytes(ipAddress.encode("utf-8"))
    )
    for index in range(0, len(joints)):
...
def getMechanicalMaxJointsVel(arrVel, ipAddress=""):
    ...
    api_mod.getMechanicalMaxJointsVel.argtypes = [
        POINTER(DIANA_JOINT_ANGULAR_VEL),
        c_char_p,
    ]
    api_mod.getMechanicalMaxJointsVel.restype = c_int
    ret = api_mod.getMechanicalMaxJointsVel(
        byref(jointsVel), bytes(ipAddress.encode("utf-8"))
    )
...
def getMechanicalMaxJointsAcc(arrAcc, ipAddress=""):
    ...
    ret = api_mod.getMechanicalMaxJointsAcc(
        byref(jointsAcc), bytes(ipAddress.encode("utf-8"))
    )
...
def getMaxCartRotationVel(ipAddress=""):
    ...
    ret = api_mod.getMaxCartRotationVel(
        byref(cartRotationVel), bytes(ipAddress.encode("utf-8"))
    )
...
def getMaxCartRotationAcc(ipAddress=""):
    ...
    ret = api_mod.getMaxCartRotationAcc(
        byref(cartRotationAcc), bytes(ipAddress.encode("utf-8"))
    )
...
def setJointsPositionRange(arrMinPos, arrMaxPos, ipAddress=""):
    ...
    ret = api_mod.setJointsPositionRange(
        jointMin, jointMax, bytes(ipAddress.encode("utf-8"))
    )
    return message(ret)
...
def setMaxCartTranslationVel(TranslationVel, ipAddress=""):
    ...
    ret = api_mod.setMaxCartTranslationVel(
        TranslationVel, bytes(ipAddress.encode("utf-8"))
    )
    return message(ret)
...
def requireHandlingError(ipAddress=""):
    ...
    ret = api_mod.requireHandlingError(
        byref(handling_error), bytes(ipAddress.encode("utf-8"))
    )
    if ret == 0:
...
def setJointsSoftLimitRange(arrMinPos, arrMaxPos, ipAddress=""):
    ...
    ret = api_mod.setJointsSoftLimitRange(
        jointMin, jointMax, bytes(ipAddress.encode("utf-8"))
    )
    return message(ret)


def getFunctionOptI4(
    funtion_index: function_index_e, opt_name: function_opt_name_e, ipAddress=""
):
    opt_value = c_int()
...
def setFunctionOptI4(
    funtion_index: function_index_e,
    opt_name: function_opt_name_e,
    opt_value,
    ipAddress="",
):
...
def getHeartbeatParam(disconnectTimeout, stopRobotTimeout, ipAddress=""):
    ...
    ret = api_mod.getHeartbeatParam(
        byref(disconnectTimeout),
        byref(stopRobotTimeout),
        bytes(ipAddress.encode("utf-8")),
    )
    return ret
...
def getCustomRobotState(customBits, ipAddress=""):
    ...
    ret = api_mod.getCustomRobotState(
        byref(customBits), bytes(ipAddress.encode("utf-8"))
    )
    return ret
...
def getTcpPoseByWorkPieceName(workPieceName, arrPose, ipAddress=""):
    pose = DIANA_TCP_POSE()
    pose.values = (c_double * 6)()
    api_mod.getTcpPoseByWorkPieceName.argtypes = [
        c_char_p,
        POINTER(DIANA_TCP_POSE),
        c_char_p,
    ]
...
def getPayLoadByTcpName(tcpName, payload, ipAddress=""):
    tcpPayload = DIANA_TCP_PAYLOAD()
    tcpPayload.values = (c_double * 10)()
    api_mod.getPayLoadByTcpName.argtypes = [
        c_char_p,
        POINTER(DIANA_TCP_PAYLOAD),
        c_char_p,
    ]
...
def getDefaultWorkPieceCoordinate(ipAddress=""):
    ...
    ret = api_mod.getDefaultWorkPieceCoordinate(
        workpieceName, bytes(ipAddress.encode("utf-8"))
    )
